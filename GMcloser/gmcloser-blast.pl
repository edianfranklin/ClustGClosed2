...
my $blast_thread = 1;
if ($target_size > 800000000){
    if ($thread_num > 5){
        $blast_thread = $thread_num - 4;
    }
    $blast_thread = 12 if ($blast_thread > 12);
}
elsif ($target_size > 500000000){
    if ($thread_num > 5){
        $blast_thread = $thread_num - 4;
    }
    $blast_thread = 8 if ($blast_thread > 8);
}
elsif ($target_size > 200000000){
    if ($thread_num > 5){
        $blast_thread = $thread_num - 4;
    }
    $blast_thread = 4 if ($blast_thread > 4);
}
$blast_thread = $thread_num if (@read_files == 0);
$blast_thread  = 0 if ($align_file ne '');
...
my $bowtie_thread = 2;
if (@read_files > 0){
    $blast_thread = 1 if (($thread_num - $blast_thread) / (@read_files * 2) < 2);
    $blast_thread = 3 if (($thread_num - $blast_thread) / (@read_files * 2) >= 3);
}
...
if (@read_files > 0){
    if ($align_file_flag == 0){
        ...
        if (($thread_num - $blast_thread) / (@read_files * 6) >= 2 and $Config{useithreads}){
            my $file_div_num = 2;
            $file_div_num = 3 if (($thread_num - $blast_thread) / (@read_files * 6) >= 3);
            $file_div_num = 4 if (($thread_num - $blast_thread) / (@read_files * 6) >= 4);
...
# Align between target and query contigs and align with short reads to target and query reference
        ...
        if ($Config{useithreads}){
            ...
            if (@read_files_2 * 2 <= $thread_num){
                ...
                if ($blast_thread > 0){
                    if ($align_file eq ''){
                        my ($thread1) = threads->new(\&blast_align_thread, $num);
                        push @align_jobs, $thread1;
                    }
                }
                ...
                foreach (@align_jobs){
                    ...
                    print STDERR "BLASTn alignmnet --- completed:\n" if ($num1 == 0);
...
sub blast_align_thread{
    my $alignum = shift;
    my $align_file = "$out_prefix-1m.blast.out";
    my $blast_dir = 'db';
    system ("mkdir $blast_dir") unless (-d $blast_dir);
    system ("cp -f $target_subcon_file $blast_dir/");
    system ("makeblastdb -in $blast_dir/$target_basename -dbtype nucl");
    system ("blastn -db $blast_dir/$target_basename -query $query_file -out $align_file -outfmt 6 -num_alignments 100 -perc_identity 95 -num_threads $blast_thread");
    threads->yield();
    sleep 1;
    return ($alignum, $align_file, 'N');
}
...
sub blast_align{
    ...
    system ("makeblastdb -in $blast_dir/$target_basename -dbtype nucl");
    system ("blastn -db $blast_dir/$target_basename -query $query_file -out $align_file -outfmt 6 -num_alignments 100 -perc_identity 95 -num_threads $blast_thread");
    print STDERR "BLAST alignment was completed:\n";
    return ($align_file);
}
                ...
                if (($pre_num > 0) and ($unallowed_subcon_flag == 0)){         # check whether the aligned target subsequences are discontinuous
                    if ($num > $pre_num + 1){
                        for (my $i = $pre_num + 1; $i < $num; $i++){
                            ...
                            if (length $target_subcon_seq{$subcon_name} > 300){
                                $unallowed_subcon_flag = 1;
                                last;
                            }
                            else{
...
sub filter_blast_result{                        # filter blast results
    ...
    open (OUT1, "> $out_prefix-2m.blast-filt.txt");
    print OUT1 '-' x 110, "\n";
