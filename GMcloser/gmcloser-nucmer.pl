...
die "The value specified with --insert should be read_len < insert < 20001: $!\n" if ($insert_size <= $read_length) or ($insert_size > 20000);
...
print OUTLOG " [nucmer alignment file]: $align_file\n" if ($align_file ne '');
...
print OUTLOG " [read length]: $read_length [insert]: $insert_size [sd_insert]: $SD_insert [min_gap]: $min_gap_size [max_indel]: $max_indel_size [nuc_len]: $min_nucmer_match [hetero]: $heterozygosity [thread]: $thread_num\n";
print OUTLOG " [min_match_len]: $min_match_length [min_identity]: $min_identity\n" if (@sam_target_align_file == 0) and (@sam_query_align_file == 0) and (@read_files == 0) and ($sam_dir eq '');
...
print STDERR " [nucmer alignment file]: $align_file\n" if ($align_file ne '');
...
print STDERR " [read length]: $read_length [insert]: $insert_size [sd_insert]: $SD_insert [min_gap]: $min_gap_size [max_indel]: $max_indel_size [nuc_len]: $min_nucmer_match [hetero]: $heterozygosity [thread]: $thread_num\n";
...
while (my $line = <FILE>){
    ...
    if ($line =~ /^>/){
        ...
        die "Target scaffold name must not contain '||' '==' or '/INT$', where 'INT' indicates integer and '$' the end of the string." if (($target_name =~ /\|\|/) or ($target_name =~ /==/) or ($target_name =~ /\/\d+$/));
    }
    else{
        $Tseq .= $line;
    }
...
if ($min_nucmer_match eq 'auto'){
    $min_nucmer_match = 30;
    $min_nucmer_match = 40 if ($target_size >= 200000000);
    $min_nucmer_match = 50 if ($target_size >= 300000000);
}
...
my @my_files = <$temp_dir/$out_prefix*>;
system "rm $temp_dir/$out_prefix*" if @my_files > 0;


my $bowtie_thread = 2;
if (@read_files > 0){
    $bowtie_thread = 1 if ($thread_num / (@read_files * 2) < 2);
    $bowtie_thread = 3 if ($thread_num / (@read_files * 2) >= 3);
}
...
if ($align_file eq ''){
    print STDERR "Nucmer alignment proceeded\n";
    system ("$Bin/Nucmer_contig_align.pl -r $target_subcon_file -q $query_file -mi 97 -p $out_prefix-1m.nucmer -l $min_nucmer_match -n $thread_num");
    $align_file = "$out_prefix-1m.nucmer.coords";
}
...
# Align between target and query contigs and align with short reads to target and query reference
        ...
        if ($Config{useithreads}){
            if ($thread_num > 1){
                ...
                my $thread_index2 = threads->new(\&bowtie_index, $query_file, $query_basename);
                push @index_jobs, $thread_index1, $thread_index2;
                foreach (@index_jobs){
                    $_->join;
                }
                undef @index_jobs;
                ...
                system ("bowtie2-build -q -f $target_subcon_file $bowtie_dir/$index_dir/$target_basename 2>/dev/null");
                system ("bowtie2-build -q -f $query_file $bowtie_dir/$index_dir/$query_basename 2>/dev/null");
            } 
            if (@read_files_2 * 2 <= $thread_num){
                my $num = 0;
                ...
                foreach (@align_jobs){
                    ...
                    push @nucmer_subfiles, $file_name if ($tag eq 'N');
                    push @sam_target_align_file, $file_name if ($tag eq 'T');
                    push @sam_query_align_file, $file_name if ($tag eq 'Q');
                    print STDERR "Bowtie alignment of read file $num1 with target contigs --- completed:\n" if ($num1 > 0) and ($tag eq 'T');
                    print STDERR "Bowtie alignment of read file $num1 with query contigs --- completed:\n" if ($num1 > 0) and ($tag eq 'Q');
                }
                undef @align_jobs;
            }
            else{
                ...
                foreach my $reads (@read_files_2){
                    $num ++;
                    system ("bowtie2 --$base_call_type -p $thread_num -x $bowtie_dir/$index_dir/$target_basename $reads | $Bin/coval-filter-short.pl -n $max_mismatch_num_coval -r $target_subcon_file - > $bowtie_dir/$out_prefix-target-align-se-$num.sam");
                    print STDERR "Bowtie alignment of read file $num with target --- finished:\n";
                    push @sam_target_align_file, "$bowtie_dir/$out_prefix-target-align-se-$num.sam";
                    system ("bowtie2 --$base_call_type -p $thread_num -x $bowtie_dir/$index_dir/$query_basename $reads | $Bin/coval-filter-short.pl -n $max_mismatch_num_coval -r $query_file - > $bowtie_dir/$out_prefix-query-align-se-$num.sam");
                    print STDERR "Bowtie alignment of read file $num with query --- finished:\n";
                    push @sam_query_align_file, "$bowtie_dir/$out_prefix-query-align-se-$num.sam";
                }
            }
        }
        else{
            print STDERR "Your Perl has not been compiled to use 'threads' module. So, PE-read alignments will be processed at a single CPU.\n";
            system ("bowtie2-build -q -f $target_subcon_file $bowtie_dir/$index_dir/$target_basename 2>/dev/null");
            system ("bowtie2-build -q -f $query_file $bowtie_dir/$index_dir/$query_basename 2>/dev/null");
            my $num = 0;
            foreach my $read_file (@read_files){
                ...
                system ("bowtie2 --$base_call_type -p $thread_num -x $bowtie_dir/$index_dir/$target_basename $read_file_2 | $Bin/coval-filter-short.pl -n $max_mismatch_num_coval -r $target_subcon_file - > $bowtie_dir/$out_prefix-target-align-se-$num.sam");
                print STDERR "Bowtie alignment of all read files with target --- finished:\n";
                push @sam_target_align_file, "$bowtie_dir/$out_prefix-target-align-se-$num.sam";
                system ("bowtie2 --$base_call_type -p $thread_num -x $bowtie_dir/$index_dir/$query_basename $read_file_2 | $Bin/coval-filter-short.pl -n $max_mismatch_num_coval -r $query_file - > $bowtie_dir/$out_prefix-query-align-se-$num.sam");
                print STDERR "Bowtie alignment of all read files with query --- finished:\n";
                push @sam_query_align_file, "$bowtie_dir/$out_prefix-query-align-se-$num.sam";
            }
        }
        my @file_list = <$temp_dir/$out_prefix-read.[0-9].f[aq]>;
        system ("rm -f $temp_dir/$out_prefix-read.[0-9].f[aq]") if (@file_list > 0);
    }
}
...
sub bowtie_index{
    my $ref_file = shift;
    my $ref_base = shift;
    system ("bowtie2-build -q -f $ref_file $bowtie_dir/$index_dir/$ref_base 2>/dev/null");
    threads->yield();
    sleep 1;
}
...
sub read_align{
    ...
    my $ref_tag = shift;
    if ($ref_tag eq 'T'){
        system ("bowtie2 --$base_call_type -x $bowtie_dir/$index_dir/$target_basename -p $bowtie_thread $readfile | $Bin/coval-filter-short.pl -n $max_mismatch_num_coval -r $target_subcon_file - > $bowtie_dir/$out_prefix-target-align-se-$readnum.sam");
    }
    elsif ($ref_tag eq 'Q'){
        system ("bowtie2 --$base_call_type -x $bowtie_dir/$index_dir/$query_basename -p $bowtie_thread $readfile | $Bin/coval-filter-short.pl -n $max_mismatch_num_coval -r $query_file - > $bowtie_dir/$out_prefix-query-align-se-$readnum.sam");
    }
    threads->yield();
    sleep 1;
    return ($readnum, "$bowtie_dir/$out_prefix-target-align-se-$readnum.sam", 'T') if ($ref_tag eq 'T');
    return ($readnum, "$bowtie_dir/$out_prefix-query-align-se-$readnum.sam", 'Q') if ($ref_tag eq 'Q');
}
...
if ($PE_align_tag == 1){
    ...
    my $ave_query_coverage = int ($query_read_num * $read_length / $total_query_len * 10) / 10;
...
# get consistent alignment info from a nucmer alignment file

my %query_cov;
my %partial_match_5;

open (FILE, $align_file) or die "$align_file is not found: $!\n";
while (my $line = <FILE>){
    chomp $line;
    my @line = split (/\s+/, $line);
    my $tstart = $line[0];
    ...
    ${${${$query_cov{$qname}}{$tname}}{$strand}}{$tstart} = "$tend=$qrange" if ($identity >= 98) and ($heterozygosity == 0);
    ${${${$query_cov{$qname}}{$tname}}{$strand}}{$tstart} = "$tend=$qrange" if ($identity >= 97) and ($heterozygosity == 1);
...
&filter_nucmer_result ();
%target_info = ();

my %query_covered;
foreach my $qname (sort keys %query_cov){           # store query with >= $max_query_singletone_cov (60%) overlapped with target subcontigs 
    foreach my $tname (sort keys %{$query_cov{$qname}}){
        foreach my $strand (keys %{${$query_cov{$qname}}{$tname}}){
            my $sum_tmatch = 0;
            my $sum_tmatch_2 = 0;
            my $pre_qstart = 0;
            my $pre_qend = 0;
            foreach my $tstart (sort {$a <=> $b} keys %{${${$query_cov{$qname}}{$tname}}{$strand}}){
                my ($tend, $qstart, $qend) = split (/=/, ${${${$query_cov{$qname}}{$tname}}{$strand}}{$tstart});
                if (($strand eq 'Plus') and (($qstart + 50 > $pre_qend) or ($pre_qend == 0))){
                    $sum_tmatch += $tend - $tstart + 1;
                }
                elsif (($strand eq 'Minus') and (($qend - 50 < $pre_qstart) or ($pre_qstart == 0))){
                    $sum_tmatch += $tend - $tstart + 1;
                }
                else{
                    $sum_tmatch_2 += $tend - $tstart + 1;
                }
                $pre_qstart = $qstart;
                $pre_qend = $qend;
            }
            my $qcov_1 = int ($sum_tmatch / length ($query_seq{$qname}) * 100);
            my $qcov_2 = int ($sum_tmatch_2 / length ($query_seq{$qname}) * 100);
            $qcov_1 = 100 if ($qcov_1 > 100);
            $qcov_2 = 100 if ($qcov_2 > 100);
            my $qcov = $qcov_1;
            $qcov = $qcov_2 if ($qcov_2 > $qcov_1);
            if ($qcov > $max_query_singletone_cov){
                $query_covered{$qname} = $qcov;
            }
        }
    }
}
...
if ($PE_align_tag == 1){
    ...
    foreach my $align (keys %evaluate_pairs){
        ...
        if ($rate_PE >= 0){
            ...
            $evaluate_pairs{$align} = "$score=$rate_PE";
        }
        else{
    ...
    $threshold_score = $standard_thresfold_score - (4.5 - $ave_log_score);
...
if ($PE_align_tag == 1){
    ...
    foreach my $align (keys %evaluate_pairs){
        my $qunmatch_len = 0;
        my ($qname, $qstart, $qend, $tname, $tstart, $tend, $strand, $overlen, $identity) = split (/==/, $align);
        my ($score, $ratePE) = split (/=/, $evaluate_pairs{$align});
        if (($score >= $threshold_score) or (($overlen >= $min_match_length) and ($identity >= $min_identity)) or ($ratePE >= 0.5)){
            my $Qlen = length $query_seq{$qname};
            my $Tlen = length $target_subcon_seq{$tname};
            ...
            ${${$Q_match{$qname}}{$tname_base}}{$tname_num} = $overlen;
            if ($tstart <= 3){
...
my %Q_match_2;
foreach my $qname (keys %Q_match){
    foreach my $tbase (keys %{$Q_match{$qname}}){
        my $pre_tnum = 0;
        my $overlap_num = 0;
        my $overlap_len = 0;
        foreach my $tnum (sort {$a <=> $b} keys %{${$Q_match{$qname}}{$tbase}}){
            if ($pre_tnum == 0){
                $overlap_num ++;
                $overlap_len += ${${$Q_match{$qname}}{$tbase}}{$tnum};
                $pre_tnum = $tnum;
                next;
            }
            else{
                if ($tnum == $pre_tnum + 1){
                    ...
                    if (exists ${$Q_match_2{$qname}}{$tbase}){
                        my ($tnum2, $overlen2) = split (/==/, ${$Q_match_2{$qname}}{$tbase});
                        if ($tnum2 < $overlap_num){
                            ${$Q_match_2{$qname}}{$tbase} = $overlap_num * 10000 + $overlap_len;
                        }
                        elsif ($tnum2 == $overlap_num){
                            if ($overlap_len > $overlen2){
                                ${$Q_match_2{$qname}}{$tbase} = $overlap_num * 10000 + $overlap_len;
                            }
                        }
                    }
...
foreach my $qname (keys %Q_match_2){                  # remove target-query matches except for the best target-query pair with the longest overlapped sequences
    my $count = 0;
    foreach my $tname_base (sort {${$Q_match_2{$qname}}{$b} <=> ${$Q_match_2{$qname}}{$a}} keys %{$Q_match_2{$qname}}){
        $count++;
        if ($count > 1){
...
foreach my $tname (keys %TQ_cover){             # remove query alignments with different alignment strand or discontinuous subseqeunces, out of queries that cover multiple subsequences of a target scaffold
...
foreach my $tname (sort keys %T_match_ALL){         # filterng overlapped alignments with only a single overlapping subcontig
    ...
    if ($num_query > 1){
        ...
        if (scalar keys %count_A > 0){
            foreach my $mqname (%multi_query){
                if (!exists $count_A{$mqname}){
                    ...
                    if (scalar keys %count_A > 1){
                        if (exists ${$Q_match_2{$mqname}}{$base_name}){
                            my $qvalue = ${$Q_match_2{$mqname}}{$base_name};
                            if ($qvalue > $top_qvalue){
                                if ($top_qname ne ''){
                                    delete ${$T_match_ALL{$tname}}{$top_qname};
                                    delete ${$TQ_cover{$base_name}}{$top_qname};
                                    $excluded_qname{$top_qname} = $tname;
                                }
                                $top_qname = $mqname;
                                $top_qvalue = $qvalue;
                            }
            ...
            foreach my $qname5 (%count_5){
                ...
                if (exists ${$Q_match_2{$qname5}}{$base_name}){
                    my $qvalue = ${$Q_match_2{$qname5}}{$base_name};
                    if ($qvalue > $top_qvalue){
                        if ($top_qname ne ''){
                            delete ${$T_match_ALL{$tname}}{$top_qname};
                            delete ${$TQ_cover{$base_name}}{$top_qname};
        ...
        $top_qname = '';
        $top_qvalue = 0;
        if ((scalar keys %count_3 > 1) and (scalar keys %count_A == 0)){
            foreach my $qname3 (%count_3){
                next if (!exists ${$TQ_cover{$base_name}}{$qname3});
                if (exists ${$Q_match_2{$qname3}}{$base_name}){
                    my $qvalue = ${$Q_match_2{$qname3}}{$base_name};
                    if ($qvalue > $top_qvalue){
                        if ($top_qname ne ''){
                            delete ${$T_match_ALL{$tname}}{$top_qname};
                            delete ${$TQ_cover{$base_name}}{$top_qname};
...
open (OUT1, "> $out_prefix-3m.selected-align-list.txt");
print OUT1 '-' x 90, "\n";
...
foreach my $tname (sort keys %T_match_ALL){         # set query coordinates to gap regions
    ...
    foreach my $qname (sort keys %{$T_match_ALL{$tname}}){
        ...
        $used_qname{$qname} = $tname;
        if (($extend == 1) and ($num == 1)){
        ...
        elsif (($extend == 1) and ($num == @{$Tsubcon_info{$base_name}})){
...
my @Q_PEalignF_subfiles;
my @Q_PEalignR_subfiles;
my @gapA_subfiles;
my @gap5_subfiles;
my @gap3_subfiles;
my @connect_subfiles;
...
if (($thread_connect > 1) and ($Config{useithreads})){              # connect neighboring subcontigs
    ...
    foreach my $tname (sort keys %target_subcon_seq){
        ...
        if ($pre_tbase ne $tname_base){
            ...
            if ($count_tbase >= $div_num){
                ...
                my $query_PEalignF_tempout = "$temp_dir/$out_prefix-query-PE-align-F-$count_sub.txt";
                my $query_PEalignR_tempout = "$temp_dir/$out_prefix-query-PE-align-R-$count_sub.txt";
                ...
                open (OUT2Q, "> $query_PEalignF_tempout") if ($PE_align_tag == 1);
                open (OUT3Q, "> $query_PEalignR_tempout") if ($PE_align_tag == 1);
                ...
                foreach my $tname_2 (sort keys %tsubseq){
                    ...
                    if ($PE_align_tag == 1){
                        ...
                        if (exists $gap_5{$tname_2}){
                            my ($qname5, $start5, $end5, $exlen5, $strand5) = split (/==/, $gap_5{$tname_2});
                            foreach my $pos (keys %{$query_read_pos_F{$qname5}}){
                                print OUT2Q "$qname5\t$pos\t${$query_read_pos_F{$qname5}}{$pos}\n";
                            }
                            foreach my $pos (keys %{$query_read_pos_R{$qname5}}){
                                print OUT3Q "$qname5\t$pos\t${$query_read_pos_R{$qname5}}{$pos}\n";
                            }
                        }
                        if (exists $gap_3{$tname_2}){
                            my ($qname3, $start3, $end3, $exlen3, $strand3) = split (/==/, $gap_3{$tname_2});
                            foreach my $pos (keys %{$query_read_pos_F{$qname3}}){
                                print OUT2Q "$qname3\t$pos\t${$query_read_pos_F{$qname3}}{$pos}\n";
                            }
                            foreach my $pos (keys %{$query_read_pos_R{$qname3}}){
                                print OUT3Q "$qname3\t$pos\t${$query_read_pos_R{$qname3}}{$pos}\n";
                            }
                        }
                    }
                }
                ...
                close (OUT2Q) if ($PE_align_tag == 1);
                close (OUT3Q) if ($PE_align_tag == 1);
                close (OUT4);
                close (OUT5);
                close (OUT6);
                ...
                push @Q_PEalignF_subfiles, $query_PEalignF_tempout if ($PE_align_tag == 1);
                push @Q_PEalignR_subfiles, $query_PEalignR_tempout if ($PE_align_tag == 1);
    ...
    foreach my $subfile (@subcon_subfiles){
        ...
        my $Q_PEalignF_subfile = $Q_PEalignF_subfiles[$num - 1] if ($PE_align_tag == 1);
        my $Q_PEalignR_subfile = $Q_PEalignR_subfiles[$num - 1] if ($PE_align_tag == 1);
        my $gapA_subfile = $gapA_subfiles[$num - 1];
        my $gap5_subfile = $gap5_subfiles[$num - 1];
        my $gap3_subfile = $gap3_subfiles[$num - 1];
        my ($thread) = threads->new(\&connect_subcon, $subfile, $num, $out_prefix, $temp_dir, $query_file, $gapA_subfile, $gap5_subfile, $gap3_subfile, $PE_align_tag, $max_insert_size, $read_length, $T_PEalignF_subfile, $T_PEalignR_subfile, $Q_PEalignF_subfile, $Q_PEalignR_subfile) if ($PE_align_tag == 1);
    ...
    system ("rm -f $temp_dir/$out_prefix-subcon-seq-[0-9]*");
    system ("rm -f $temp_dir/$out_prefix-target-PE-align-[FR]-[0-9]*");
    system ("rm -f $temp_dir/$out_prefix-query-PE-align-[FR]-[0-9]*");
    system ("rm -f $temp_dir/$out_prefix-[0-9]-seq[12].fa");
    system ("rm -f $temp_dir/$out_prefix-gap[A53]-[0-9]*");
            ...
            if ((exists $gap_5{$tname}) or (exists $gap_3{$down_tname})){
                ...
                (($match == 1) and ($PE_align_tag == 0) and ($match_len1 >= $min_match_len_local) and ($identity2 >= $min_identity))){
                    ...
                    if ($gap_5_unmatch_len + $gap_3_unmatch_len > $gap_size * 1.5){
                        delete $gap_5{$tname} if ($gap5_qname ne '');
                        delete $gap_3{$tname} if ($gap3_qname ne '');
                        $excluded_qname{$gap5_qname} = $tname if ($gap5_qname ne '');
                        $excluded_qname{$gap3_qname} = $tname if ($gap3_qname ne '');
                    }
                }
            }
            else{
...
my $pre_base_name = '';
my $pre_num = 1;
open (OUT, "> $out_prefix.gapclosed.fa");
foreach my $tname (sort keys %target_gap_size){         # substitute target gap regions with query and extend target termini
    ...
    if ($pre_base_name ne $base_name){
        ...
        if (($extend == 1) and (exists $term5{$pre_base_name})){
        ...
        if (($extend == 1) and (exists $term3{$pre_base_name})){
...
my $used_read_file = "$out_prefix-4m.contigs-used-for-gapfill.txt";
...
open (OUT, "> $out_prefix.query-singletone.fa");
    foreach my $qname (keys %query_seq){
        if ((!exists $included_qname{$qname}) and (!exists $excluded_qname{$qname}) and (!exists $used_qname{$qname}) and (!exists $query_covered{$qname})){
            print OUT ">$qname\n";
            print OUT $query_seq{$qname}, "\n";
        }
    }
close (OUT);

$localtime = localtime;
print STDERR "\nJob finished: $localtime\n";
...
sub filter_nucmer_result{                        # filter nucmer results
    my %target_pos;
    ...
    open (OUT1, "> $out_prefix-2m.nucmer-filt.txt");
    print OUT1 '-' x 110, "\n";
...
sub connect_subcon{
    ...
    if ($connect_subcon == 0){
        system ("$Bin/connect_subcontigs_GMcloser2.pl -s $subfile -q $queryfile -n $num -p $out_prefix_2 -td $temp_dir_2 -ga $gapA_file -gu $gap5_file -gd $gap3_file -t $pe_align_tag -mi $min_identity -mm $min_match_length -ml $min_match_len_local -i $max_insert_size_2 -r $read_length_2 -tf $T_PEalignF_file -tr $T_PEalignR_file -qf $Q_PEalignF_file -qr $Q_PEalignR_file") if ($pe_align_tag == 1);
        system ("$Bin/connect_subcontigs_GMcloser2.pl -s $subfile -q $queryfile -n $num -p $out_prefix_2 -td $temp_dir_2 -ga $gapA_file -gu $gap5_file -gd $gap3_file -t $pe_align_tag -mi $min_identity -mm $min_match_length -ml $min_match_len_local") if ($pe_align_tag == 0);
    }
    else{
        system ("$Bin/connect_subcontigs_GMcloser2.pl -s $subfile -q $queryfile -n $num -p $out_prefix_2 -td $temp_dir_2 -ga $gapA_file -gu $gap5_file -gd $gap3_file -t $pe_align_tag -mi $min_identity -mm $min_match_length -ml $min_match_len_local -i $max_insert_size_2 -r $read_length_2 -tf $T_PEalignF_file -tr $T_PEalignR_file -qf $Q_PEalignF_file -qr $Q_PEalignR_file -cn") if ($pe_align_tag == 1);
        system ("$Bin/connect_subcontigs_GMcloser2.pl -s $subfile -q $queryfile -n $num -p $out_prefix_2 -td $temp_dir_2 -ga $gapA_file -gu $gap5_file -gd $gap3_file -t $pe_align_tag -mi $min_identity -mm $min_match_length -ml $min_match_len_local -cn") if ($pe_align_tag == 0);
    }
    threads->yield();
#    sleep 1;
