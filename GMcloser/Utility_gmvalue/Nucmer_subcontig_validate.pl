...
# a script for consecutive commands of Nucmer/GAGE and correct_contigs_GAGE_coords.pl for evaluation of subcontig integrity accuracy for scaffolds
...
print STDERR "min_gap_size: $min_gap_size\n";
...
if ($min_nucmer_match == 30){
    $min_nucmer_match = 40 if ($query_length >= 200000000);
    $min_nucmer_match = 50 if ($query_length >= 300000000);
}
...
my $qdiv_len = 0;
if ($query_length > $target_length * 2){
    $qdiv_len = int ($query_length / $thread) + $thread;
}
...
if ($qdiv_len == 0){
    ...
    if (@sub_query > 0){
        if ($count_sub > 1){
            ...
            push @query_subfiles, $subcontig_file;
        }
    }
}
undef @sub_target;
...
if ($error_correct == 0){
    system ("$Bin/correct_contigs_coords.pl -q $subcontig_file -a $prefix_out.coords -r $ref_file -p $prefix_out -mi $min_identity -ma $min_align_len -ml $min_contig_len -mc $min_coverage -is $max_indel_len");
}
else{
    system ("$Bin/correct_contigs_coords.pl -q $subcontig_file -a $prefix_out.coords -r $ref_file -p $prefix_out -mi $min_identity -ma $min_align_len -ml $min_contig_len -mc $min_coverage -is $max_indel_len -e");
}

my $gap_length = $scaf_length - $query_length;
my $gap_rate = int ($gap_length / $scaf_length * 1000) / 10;
open (OUT, ">> $prefix_out.stat.txt");
print OUT "\nTotal scaffold length: $scaf_length\n";
print OUT "Gap length: $gap_length ($gap_rate%)\n";
close (OUT);
print STDERR "Gap length: $gap_length bp\n";
